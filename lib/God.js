...
God.executeApp = function executeApp(env, cb) {
  ...
  function workerMsg(msg){
    if(msg === env_copy.online_event){
      onlineHandler();
      // cluster
      if(clu.process && clu.process.removeListener){
        clu.process.removeListener('message', workerMsg)  
      }
      // fork
      else{
        clu.removeListener('message', workerMsg)
      }
    }
  }
  ...
  if (env_copy.exec_mode === 'cluster_mode') {
     ...
     */
    God.nodeApp(env_copy, function nodeApp(err, _clu) {
      ...
      clu.once('online', function cluOnline() {
        console.log(
          'App name:%s id:%s got online event from cluster', 
          clu.pm2_env.name, 
          clu.pm2_env.pm_id
        );
        if(!env_copy.online_event){
          return onlineHandler();
        }
        
        console.log(
          'App name:%s id:%s waiting for online_event from worker: %s', 
          clu.pm2_env.name, 
          clu.pm2_env.pm_id, 
          env_copy.online_event
        );

        clu.process.on('message',workerMsg);
      });
      return false;
    });
  }
     ...
     * Fork mode logic
     */
    God.forkMode(env_copy, function forkMode(err, _clu) {
      if (cb && err) return cb(err);
      if (err) return false;
      clu = _clu;
